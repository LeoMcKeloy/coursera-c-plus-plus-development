...
public:
    const static uint64_t N = static_cast<uint64_t>(TAirport::Last_);
  ...
  AirportCounter(TIterator begin, TIterator end) {
      for (auto it = begin; it != end; it++) {
          ++airports[static_cast<uint64_t>(*it)];
      }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
        return airports.at(static_cast<uint64_t>(airport));
    }

  // добавить данный элемент
  void Insert(TAirport airport) {
        ++airports[static_cast<uint64_t>(airport)];
    }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) {
        if (airports[static_cast<uint64_t>(airport) != 0]) {
            --airports[static_cast<uint64_t>(airport)];
        }
    }
  ...
  void EraseAll(TAirport airport) {
        airports[static_cast<uint64_t>(airport)] = 0;
    }
  ...
  Items GetItems() const {
      ...
      for (uint64_t i = 0; i < N; i++) {
          result[i] = make_pair(static_cast<TAirport>(i), airports.at(i));
      }
      return result;
  }

private:
    array<size_t, N> airports = {0};
};
